/**
 * BestsellerCard.js — Card de produto horizontal
 *
 * HTML e classes idênticos ao cardapio-antigo/render.js (createProductCard).
 * Usa as classes .product-card__* para manter consistência visual.
 */
...
export class BestsellerCard extends Component {
  render() {
    ...
    return `
      <article class="product-card" data-product-id="${escapeHtml(product.id)}" role="button" tabindex="0"
  ...
  afterMount() {
    ...
    const addBtn = this.el.querySelector('[data-action="add"]');
    if (addBtn) {
